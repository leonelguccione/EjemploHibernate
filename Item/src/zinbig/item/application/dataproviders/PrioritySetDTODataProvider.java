 ...
 * de prioridades para los listados. <br>
 * El listado puede ser paginado.
 * 
 * @author Javier Bazzocco javier.bazzocco@zinbig.com
 * 
 */
public class PrioritySetDTODataProvider extends
		SortableDataProvider<PrioritySetDTO> {

	/**
	 * UID por defecto para la serialización.
	 */
	private static final long serialVersionUID = -5557125806157374103L;
	 ...
	 *            resultado.
	 */
	public PrioritySetDTODataProvider(String aColumnName, String anOrdering) {
	 ...
	 */
	@Override
	public Iterator<PrioritySetDTO> iterator(int firstIndex, int count) {
		Collection<PrioritySetDTO> result;

		try {
			SortParam sp = this.getSort();
			String ordering = sp.isAscending() ? "ASC" : "DESC";

			result = this.getPrioritiesService().getPrioritySets(firstIndex,
					count, sp.getProperty(), ordering);

		} catch (Exception e) {
			result = new ArrayList<PrioritySetDTO>();
			e.printStackTrace();
		}
	 ...
	 */
	@Override
	public IModel<PrioritySetDTO> model(PrioritySetDTO aDto) {

		return new Model<PrioritySetDTO>(aDto);
	}
	...
	public int size() {
		...
		try {
			return this.getPrioritiesService().getPrioritySetCount();
	 ...
	 *         relacionados con los conjuntos de prioridades.
	 */
	protected PrioritiesServiceBI getPrioritiesService() {
