 ...
 * usuarios para los listados. <br>
 * El listado puede ser paginado.
 * 
 * @author Javier Bazzocco javier.bazzocco@zinbig.com
 * 
 */
public class UserDTODataProvider extends SortableDataProvider<UserDTO> {

	/**
	 * UID por defecto para la serialización.
	 */
	private static final long serialVersionUID = 4588315559837026334L;
	 ...
	 * 
	 */
	public UserDTODataProvider(String aColumnName, String anOrdering) {
	 ...
	 */
	@Override
	public Iterator<UserDTO> iterator(int firstIndex, int count) {
		...
		try {
			...
			result.addAll(this.getUsersService().getUsers(firstIndex, count,
					sp.getProperty(), ordering));
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 */
	@Override
	public IModel<UserDTO> model(UserDTO aDto) {

		return new Model<UserDTO>(aDto);
	}

	/**
	 * Getter.
	 * 
	 * @return el tamaño de la colección de elementos recupedos por esta clase.
	 */
	@Override
	public int size() {
		int result = 0;
		...
		try {
			result = this.getUsersService().getUsersCount();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return result;
